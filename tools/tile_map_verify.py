...
# Create font object to draw tile IDs
...
for i in tqdm(range(num_tiles)):
    ...
    try:
        with Image.open(tile_path) as tile_image:
            ...
            # Draw tile ID in top left corner
            draw = ImageDraw.Draw(tile_map)
            draw.text((tile_x * tile_size, tile_y * tile_size), str(tile_id), fill="purple", font=font)
    except FileNotFoundError as e:
        print(f"Error: {e}")
        continue

# Save tile map to output directory
tile_map_path = os.path.join(output_dir, "tile_map_ids.png")
